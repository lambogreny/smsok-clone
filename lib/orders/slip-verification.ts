...
function getExpectedSlipAmount(order: QueuedOrderRecord) {
  return order.hasWht ? toNumber(order.payAmount) : toNumber(order.totalAmount);
}
...
function buildRejectResolution(
  code: SlipRejectCode,
  fallbackMessage?: string,
): SlipRejectResolution {
  switch (code) {
    case "DUPLICATE_SLIP":
      return { code, message: "สลิปนี้ถูกใช้แล้ว" };
    case "INVALID_SLIP":
      return { code, message: fallbackMessage ?? "สลิปไม่ถูกต้องหรือไม่สามารถตรวจสอบได้" };
    case "AMOUNT_MISMATCH":
      return { code, message: "จำนวนเงินในสลิปไม่ตรงกับยอดที่ต้องชำระ" };
    case "EXPIRED_SLIP":
      return { code, message: "สลิปเกินเวลา 48 ชั่วโมง กรุณาใช้สลิปใหม่" };
    case "WRONG_ACCOUNT":
      return { code, message: "บัญชีปลายทางในสลิปไม่ตรงกับบัญชีบริษัท" };
    case "UNREADABLE_SLIP":
      return { code, message: "สลิปไม่ชัดเจนหรือไม่สามารถอ่าน QR ได้" };
  }
}

function resolveUnreadableSlip(verification: SlipVerifyResult) {
  const raw = `${verification.providerCode ?? ""} ${verification.error ?? ""}`.toLowerCase();
  return (
    raw.includes("unread") ||
    raw.includes("invalid_image") ||
    raw.includes("slip_not_found") ||
    raw.includes("qr")
  );
}

function getDeterministicRejectResolution(verification: SlipVerifyResult): SlipRejectResolution {
  if (verification.isDuplicate) {
    return buildRejectResolution("DUPLICATE_SLIP");
  }

  if (verification.providerCode === "1013") {
    return buildRejectResolution("AMOUNT_MISMATCH");
  }

  if (verification.providerCode === "1014") {
    return buildRejectResolution("WRONG_ACCOUNT");
  }

  if (resolveUnreadableSlip(verification)) {
    return buildRejectResolution("UNREADABLE_SLIP");
  }

  return buildRejectResolution("INVALID_SLIP", verification.error ?? undefined);
}
...
async function rejectQueuedOrderSlip(
  ...
  rejection: SlipRejectResolution,
) {
  if (!order.slip) {
    return {
      status: "ignored" as const,
...
async function approveQueuedOrderSlip(
  ...
  if (duplicateSlip) {
    return rejectQueuedOrderSlip(
      ...
      buildRejectResolution("DUPLICATE_SLIP"),
    );
  }
...
export async function processQueuedOrderSlipVerification(input: {
  ...
  if (verification.isDuplicate) {
    return rejectQueuedOrderSlip(
      ...
      getDeterministicRejectResolution(verification),
    );
  }
  ...
  const slipDate = new Date(verification.data.date);
  if (Number.isNaN(slipDate.getTime())) {
    return rejectQueuedOrderSlip(
      order,
      verification.data as unknown as Prisma.JsonValue,
      buildRejectResolution("INVALID_SLIP"),
    );
  }

  if (Date.now() - slipDate.getTime() > SLIP_MAX_AGE_MS) {
    return rejectQueuedOrderSlip(
      order,
      verification.data as unknown as Prisma.JsonValue,
      buildRejectResolution("EXPIRED_SLIP"),
    );
  }

  const receiverCheck = checkReceiverAccount(verification.data.receiver.account);
  if (!receiverCheck.matches) {
    if (receiverCheck.error === "receiver_verification_unavailable") {
      return moveOrderToManualReview(
        order,
        verification,
        "Receiver account verification unavailable",
      );
    }
    ...
    return rejectQueuedOrderSlip(
      order,
      verification.data as unknown as Prisma.JsonValue,
      buildRejectResolution(
        receiverCheck.error === "receiver_account_missing" ? "UNREADABLE_SLIP" : "WRONG_ACCOUNT",
      ),
    );
  }

  const amountDifference = Math.abs(verification.data.amount - getExpectedSlipAmount(order));
  if (amountDifference > 1) {
    return rejectQueuedOrderSlip(
      order,
      verification.data as unknown as Prisma.JsonValue,
      buildRejectResolution("AMOUNT_MISMATCH"),
    );
  }
