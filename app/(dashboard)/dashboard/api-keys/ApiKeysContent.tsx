...
export default function ApiKeysContent({ userId, apiKeys: initialKeys }: { userId: string; apiKeys: ApiKey[] }) {
  ...
  return (
    <div className="p-6 md:p-8 max-w-6xl animate-fade-in">
      ...
      {newlyCreatedKey && (
        <div className="glass p-6 mb-6 border-emerald-500/20">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/5">
                  ...
                  <th className="text-left px-5 py-3 text-xs text-white/40 uppercase tracking-wider font-medium">สร้างเมื่อ</th>
                  <th className="text-left px-5 py-3 text-xs text-white/40 uppercase tracking-wider font-medium">ใช้ล่าสุด</th>
              ...
              <tbody>
                {apiKeys.map((key) => (
                  <tr key={key.id} className="table-row">
                    ...
                    <td className="px-5 py-3.5 text-white/30 text-xs">{new Date(key.createdAt).toLocaleDateString("th-TH")}</td>
                    <td className="px-5 py-3.5 text-white/30 text-xs">{key.lastUsed ? new Date(key.lastUsed).toLocaleDateString("th-TH") : "ยังไม่เคยใช้"}</td>
                    <td className="px-5 py-3.5">
