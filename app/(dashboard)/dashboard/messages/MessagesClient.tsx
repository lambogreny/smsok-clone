          ...
          <div className="px-4 pb-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} onClick={(e) => { try { (e.currentTarget as HTMLInputElement).showPicker(); } catch {} }} style={{ colorScheme: "dark" }} className="h-11 text-sm bg-[var(--bg-base)] border-[var(--border-default)] text-[var(--text-primary)] rounded-lg cursor-pointer" />
              </div>
              <div>
                <label className="text-xs font-medium text-[var(--text-muted)] uppercase tracking-wider mb-2 block">ถึง</label>
                <Input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} onClick={(e) => { try { (e.currentTarget as HTMLInputElement).showPicker(); } catch {} }} style={{ colorScheme: "dark" }} className="h-11 text-sm bg-[var(--bg-base)] border-[var(--border-default)] text-[var(--text-primary)] rounded-lg cursor-pointer" />
              </div>
            </div>
